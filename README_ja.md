...
### json3pdfB.py (最終ファイルの調整用に黒文字PDFファイルを作成)
...
現時点では、異なるレイアウトブロックに対して異なるフォントとサイズをサポートしていません。縦書きも非対応。あとで対応するかもだけど、検索可能にするのが主目的でタイトルとかチャプターとかの大文字、縦書きは大体の位置は合ってて検索には問題ないので優先順位は低い。

### mergejsB.py (調整用の最適化されたPDFファイルと黒文字PDFファイルをマージ)
...
現時点では、縦書きテキストレイアウトはサポートされていません（横書きテキストとして追加します）。

### json3pdfC (検索可能なテキストレイヤー用にクリアテキストPDFファイルを作成)

1. スクリプトを実行します。これにより、_ClearText PDFファイルが出力フォルダ(./OCRclearPDF)に出力されます。
   調整したjson3pdfBと同じオプション指定を。

### mergejsC (最終出力用に元のPDFファイルとクリアテキストPDFファイルをマージ)

1. スクリプトを実行します。これにより、_OCR検索可能なクリアテキストPDFファイルが出力フォルダ(./OCRfinalPDF)に出力されます。
   調整したmergejsBと同じオプションを指定を。
