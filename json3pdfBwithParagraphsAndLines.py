...
for json_file in json_files:
    ...
    if os.path.exists(ocr_json_path):
        ...
        for page_number, page in enumerate(analyze_result['pages'], start=1):
            ...
            for paragraph in analyze_result['paragraphs']:
                # パラグラフが現在のページに属しているか確認
                if any(br['pageNumber'] == page_number for br in paragraph['boundingRegions']):
                    bounding_box = paragraph['boundingRegions'][0]['polygon']
                    # バウンディングボックスの高さと幅を計算
                    bounding_box_height = abs(bounding_box[5] - bounding_box[1]) * INCH_TO_POINT
                    bounding_box_width = abs(bounding_box[4] - bounding_box[0]) * INCH_TO_POINT
                    ...
                    char_width = bounding_box_width / (num_chars / num_lines)
                    
                    # フォントサイズを設定（文字の縦横の長さの平均を基に）
                    font_size = math.sqrt(char_height * char_width)
