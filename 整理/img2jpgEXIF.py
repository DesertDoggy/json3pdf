...
for subdir, _, files in os.walk(input_folder):
    ...
    subfolder_path = subdir.replace(input_folder, working_folder)
    ...
    for file in files:
        ...
        if file.lower().endswith(supported_extensions):
            ...
            if file.lower().endswith(skip_conversion_extensions):
                ...
                # コンソールにメッセージを表示
                print(f'PDF compatible file. Skipped conversion and copied {file} to {working_folder}.')
                print(f'PDF対応形式。変換をスキップし、{file}を{working_folder}にコピーしました。')
            else:
                ...
                with Image.open(original_path) as img:
                    ...
                    exif_dict["0th"][piexif.ImageIFD.XResolution] = (target_dpi[0], 1)
                    exif_dict["0th"][piexif.ImageIFD.YResolution] = (target_dpi[1], 1)
                    exif_dict["0th"][piexif.ImageIFD.ResolutionUnit] = 2  # 2はインチを意味します
                    exif_bytes = piexif.dump(exif_dict)

                    img.save(output_path, 'JPEG2000', quality_mode='lossless', dpi=target_dpi, exif=exif_bytes)
